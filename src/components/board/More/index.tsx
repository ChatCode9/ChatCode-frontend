...
export default function More({ onClick, id, isUserPost = false  }: Props) {
  ...
  const handlePostUpdateBlindClick = () => {
    // Handle post update blind click action
  };

  const handlePostDeleteClick = () => {
    // Handle post delete click action
  };
  ...
  return (
    <Container $isMenuOpen={isMenuOpen} ref={dropdownRef}>
      ...
      {isMenuOpen && (
        <>
          {!isUserPost ? (
            <DropdownButton onClick={handleBlindClick}>
              선택 글 그만보기
            </DropdownButton>
          ) : (
            <>
              <DropdownButton onClick={handlePostUpdateBlindClick}>
                글 수정
              </DropdownButton>
              <DropdownButton onClick={handlePostDeleteClick}>
                글 삭제
              </DropdownButton>
            </>
          )}
